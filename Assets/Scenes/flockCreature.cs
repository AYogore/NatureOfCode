...
public class flockCreature : MonoBehaviour
{
    public float maxSpeed = 2, maxForce = 2;

    public Mesh coneMesh;
    public Mesh creaturePrefab;// If you want to use your own cone mesh, drop it into the editor here.

    private List<Ecoboid> ecoboids; // Declare a List of Vehicle objects.
    private Vector2 minimumPos, maximumPos;

    // Start is called before the first frame update
    void Start()
    {
        ecoboids = new List<Ecoboid>(); // Initilize and fill the List with a bunch of Vehicles
        for (int i = 0; i < 100; i++)
        {
            float ranX = Random.Range(-1.0f, 1.0f);
            float ranY = Random.Range(-1.0f, 1.0f);
            ecoboids.Add(new Ecoboid(new Vector2(ranX, ranY), minimumPos, maximumPos, maxSpeed, maxForce, creaturePrefab));
        }
    }
    ...
    void Update()
    {
        ...
        {
            ecoboids.Add(new Ecoboid(mousePos, minimumPos, maximumPos, maxSpeed, maxForce, coneMesh));
        }
    }
...
class Ecoboid
{
    ...
    public Ecoboid(Vector2 initPos, Vector2 _minPos, Vector2 _maxPos, float _maxSpeed, float _maxForce, Mesh coneMesh)
    {
    ...
    public void Flock(List<Ecoboid> ecoboids)
    {
        Vector2 sep = Separate(ecoboids); // The three flocking rules
        Vector2 ali = Align(ecoboids);
        Vector2 coh = Cohesion(ecoboids);
    ...
    public Vector3 Align(List<Ecoboid> ecoboids)
    {
        ...
        int count = 0;
        foreach (Ecoboid other in ecoboids)
        {
    ...
    public Vector2 Cohesion(List<Ecoboid> ecoboids)
    {
    ...
    public Vector2 Separate(List<Ecoboid> ecoboids)
    {
        Vector3 sum = Vector3.zero;
        int count = 0;
